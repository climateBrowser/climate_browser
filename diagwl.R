...
diagwl <- function(dat, est="", alt=NA, per="", margen=c(4,4,5,4),
  ...
  mtext(formatC(max(as.matrix(dat[2,])),digits=1,format="f"),2,las=1,
    line=2,at=40) #at changed from 35
  #media de las m?nimas del mes m?s fr?o
  mtext(formatC(min(as.matrix(dat[3,])),digits=1,format="f"),2,las=1,
    line=2,at=10) #at changed from 15
  #marcar l?mites de los meses:
  for(i in 0:13) segments(i,0,i,-1.5)
  #r?tulos meses:
  mtext(mlab,1,las=1,line=0.5,adj=0.5,at=x[2:13])
  
  #changed by jnjensen88, moved from above
  corners = par("usr")
  par(xpd=TRUE)
  text(x = corners[2]+1.5, y = mean(corners[3:4]), "Precipitation in mm", srt=270, col = pcol)
  text(x = corners[1]-1.5, y = mean(corners[3:4]), "Temperature in Celsius", srt = 270, col = tcol)
  text(x = mean(corners[1:2]), y = corners[1] - 8, "Month")
  #mtext("Temperature in Celsius",2,col=tcol,las=1,line=3,adj=0,at=55)
  #mtext("Precipitation in mm",4,col=pcol,las=1,line=3,adj=1,at=55)
  #end changed
